...
g = 386.1;                      % acceleration of gravity [in/sec^2]
...
for i = 2:(length(p))
    ...
    if(stage ==1)
        ...
        phat(i) = p(i)+w*u(i)/h+k*xmin+k*uy-alpha*u(i-1)-beta*u(i);%%change
        u(i+1) = phat(i)/khat;
        v(i) = (u(i+1)-u(i-1))/(2*dt);
        acc(i) = (u(i+1)-2*u(i)+u(i-1))/(dt^2);
        f(i)=k*(u(i)-xmin-uy)-w*u(i)/h;
        % Time Vector
        Time(i) = Time(i-1) + dt;
        Aground(i) = Ag(i);
        TimeEnd = i;
    ...
    if(stage==2)
        ...
        u(i) = phat(i-1)/khat;%%u1
        phat(i) = p(i)+w*u(i)/h-k*uy-alpha*u(i-1)-beta*u(i);%%change
        u(i+1) = phat(i)/khat;
        v(i) = (u(i+1)-u(i-1))/(2*dt);
        acc(i) = (u(i+1)-2*u(i)+u(i-1))/(dt^2);
        f(i)=k*uy-w*u(i)/h;
    ...
    if(stage==3)
        ...
        beta = k-2*m/dt^2;%%change
        u(i) = phat(i-1)/khat;%%u1
        phat(i) = p(i)+w*u(i)/h+k*(xmax-uy)-alpha*u(i-1)-beta*u(i);%%change
        u(i+1) = phat(i)/khat;
        v(i) = (u(i+1)-u(i-1))/(2*dt);
        acc(i) = (u(i+1)-2*u(i)+u(i-1))/(dt^2);
        f(i)=k*u(i)-k*(xmax-uy)-w*u(i)/h;
        % Time Vector
        Time(i) = Time(i-1) + dt;
        Aground(i) = Ag(i);
        TimeEnd = i;
